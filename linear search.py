...
# Create a window
window = pyglet.window.Window(width=1010, height=200, caption='Linear Search Visualization')
batch = pyglet.graphics.Batch()

# Generate a list with random numbers ensuring 42 is included
numbers = random.sample(range(1, 100), 24) + [42]
...
def on_draw():
    ...
    for i, number in enumerate(numbers):
        ...
        if i == current_index and not search_complete:
            ...
            pyglet.shapes.Triangle(x+15, y+40, x+10, y+50, x+20, y+50, color=color, batch=None, group=None,).draw()
        elif i == found_index:
